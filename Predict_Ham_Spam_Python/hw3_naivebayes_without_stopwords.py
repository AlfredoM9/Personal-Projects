...
# Class : CS 6375.001
# Date  : 10/14/20
# Descr : This is naive bayes without stop words. This program follows the instructions given in the assignment 3
...
#         uses naive bayes to calculate the probability of each doc. Whichever one is higher is the class that the
#         program has predicted for that doc. The results are then outputted.
...
# the prior probability and the conditional probability based on the vocab. This is done without stop words.
def train(classes):
    ...
    for file in total_files:
        ...
        tokens = [item for sublist in tokens for item in sublist if item not in stopwords]
