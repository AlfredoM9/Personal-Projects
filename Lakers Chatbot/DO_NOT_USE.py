...
#         will scrape the relevant text off the page, process it, and write it to a txt file. The program will then try
#         to find the most important terms using two methods. First it will use the tf-idf to get the most important
#         words of each document. Then the program will do some calculation based on the document size to determine how
#         many top-weighted words of each document to extract and add to the overall list of important words. From these
...
import math
from DO_NOT_USE_SQL import create_connection, create_table, create_list_entry, get_db_data


# Main function
def main(start_url="https://tinyurl.com/nbagooglesearch", wiki=False, keywords=["nba"]):
    # Get urls: Q1
    url_db_name = get_urls(start_url, wiki, keywords)

    get_data(url_db_name)

    # Get important terms: Q4
    tf_idf_total = get_tf_idf()
    important_terms = get_important_terms(tf_idf_total)
    terms_db_name, total_terms_db = store_important_terms(important_terms)
    ...
    # Print results
    print_results(url_db_name, total_terms_db, kb_db_name)
...
Descr: Starts with a google search and crawls through the web to obtain relevant urls. The urls and then stored into
       a database file. 
"""
def get_urls(start_url, wiki, keywords):
    ...
    for url in urls:
        ...
        for link in data_soup.find_all('a'):
            ...
            revelant = [True for kyword in keywords if kyword in link_str]
            # Link must have lakers in it
            if True in revelant:
                if wiki and 'wiki' in link_str:
                    continue
...
Retur: None
Descr: Iterates through the urls and extracts the data
"""
def get_data(url_db_name):
    # Get the urls from the db file passed
    db_conn = create_connection(url_db_name)
    db_data = get_db_data(db_conn, url_db_name[:-3])
    db_conn.close()

    # If a data folder does not exists, then make it to contain data text
    ...
    for id, element in db_data:
        try:
            ...
            file = open('./data/' + str(file_name), 'w+', encoding='utf-8')
...
def get_tf_idf():
    ...
    # Get the file names from the data folder
    ...
    for filename in filenames:
        ...
        file = open('./data/' + str(filename), 'r', encoding='utf-8')
...
def store_important_terms(important_terms):
    ...
    total_db_name = "total_important_terms.db"
    tb_name = db_name[:-3]
    total_tb_name = total_db_name[:-3]
    ...
    db_conn = create_connection(db_name)
    total_db_conn = create_connection(total_db_name)

    # If connection isn't establish, exit program
    if db_conn is None or total_db_conn is None:
    ...
    terms_create_table_query = "CREATE TABLE IF NOT EXISTS " + str(tb_name) + "(id integer PRIMARY KEY, element text " \
                                                                       "NOT NULL); "
    total_terms_create_table = "CREATE TABLE IF NOT EXISTS " + str(total_tb_name) + "(id integer PRIMARY KEY, element text " \
                                                                       "NOT NULL); "

    # Create table
    create_table(db_conn, terms_create_table_query)
    create_table(total_db_conn, total_terms_create_table)

    # Create entries in the table with the important terms
    create_list_entry(db_conn, important_terms, tb_name)
    create_list_entry(total_db_conn, important_terms, total_tb_name)
...
def create_knowledge_base(terms_db_name):
    ...
    for id, term in db_data:
        ...
        for filename in filenames:
            # Open the file and read it
            input_file = open('./data/' + str(filename), 'r', encoding='utf-8')
...
Descr: Filters and processes the list of strings to get rid of html tags. It then sentence tokenize the strings to be
       able to write one sentence to each line to the file passed. This is a helper function to the get_data
       function.
"""
